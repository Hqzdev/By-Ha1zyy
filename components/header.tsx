...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-center bg-white/50 py-3 backdrop-blur-md">
      <div className="flex h-[48px] items-center rounded-full bg-gray-100/90 p-1 backdrop-blur-sm">
        ...
        >
          ...
          <Link
            href="#projects"
            className={`relative rounded-full px-4 py-1.5 text-sm font-medium text-gray-600 transition-colors duration-300 ${
              hoveredTab === "projects" ? "text-white" : ""
            }`}
          ...
          <Link
            ...
            className={`relative rounded-full px-4 py-1.5 text-sm font-medium text-gray-600 transition-colors duration-300 ${
              hoveredTab === "github" ? "text-white" : ""
            }`}
            target="_blank"
            rel="noopener noreferrer"
            onMouseEnter={(e) => handleLinkHover(e, "github")}
          >
            GitHub
          </Link>
        </nav>
